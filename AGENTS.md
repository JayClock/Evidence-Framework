...
- 显式建模从 `/evidence-model` 进入，交由 `evidence-modeling`。访谈默认边澄清边沉淀领域语言：先保存原话与来源，再按 FM 方法当轮更新唯一 JSON 词汇表 `.evidence/glossary.json`；不扩大到模型 JSON、规则、场景或 API 编辑，明确的只读／仅记录要求优先。问答工具每次只问一个业务问题，记录与术语任一保存失败就停止，分别交接保存结果。
