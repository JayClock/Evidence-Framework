...
每个任务 CHECK 明确目的、被测行为、真实依赖/Fake、固定输入与业务时间、正常/边界/反例、失败不变性、测试文件、cwd、精确命令和环境准备。`acceptanceCriteria` 必须引用本任务 CHECK，并用 `assertions[{path, operator, expected}]` 保存具体、保留类型的验收数据。命令未知填 null 并关联局部 gap，不写“运行相关测试”冒充可执行。
