...
这个坏味道的根源是写“怎么做”比写“怎么检查”容易：“亮点要带数据”是过程描述，写起来轻松；“检查亮点是否包含数字、百分比或对比”要求先想清楚结果长什么样。项目选择了后者作为默认方向。
...
| 前馈                                     | 反馈                                                                                                                                        |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| FM 业务规则（25 条 CEL）                 | `validation/instances/` 27 份回放单据与 18 个正常/边界/异常场景；`check_fm.py` 的只读校验与单据模拟                                         |
| 用户故事与验收（US/AC）                  | 任务 `checks`：固定输入、预期、失败不变性、精确命令与 `evidenceRequired`                                                                    |
| 工序退出条件                             | `completionCriteria` 逐项对应具体 CHECK；模板禁止用通用工序、文档存在或自评替代                                                             |
...
| 索引/任务模板契约与单一状态归属          | planning 与 delivery 的 [Guides 契约测试](../../.agents/skills/evidence-task-planning/tests/test_guides_contract.py)                        |
