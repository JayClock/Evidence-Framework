...
上下文倾倒指的是把能找到的资料不加筛选全部塞给 Agent。本项目有现成的倾倒路径：[`.evidence/fm/`](../../.evidence/fm/) 约 1.1 MB、[`api.json`](../../.evidence/api/api.json) 约 388 KB、[`plan.yaml`](../plans/smart-domain/plan.yaml) 约 280 KB，再加上 [`openwiki/`](../../openwiki/index.md) 约 504 KB 和维护范围内的全部指南文档。让 Agent“先把仓库读一遍”再动手，就是一次标准的上下文倾倒：窗口被填满，真正决定这次改动的约束与 CHECK 被挤到边缘。
...
成因与课程一致：上下文窗口有限，关键约束会被挤出或被压缩；注意力稀释，CHECK、非目标、gap 与无关资料同权竞争。诊断信号是查找动作远多于实施动作，且查找范围超出本次任务的 `sourceRefs`：读到了与任务无关的内容，把 FM/API 里存在的活动当成任务，却说不出“不给它，Agent 会做错哪个决策”。

## Solution

项目把上下文当预算用，落到四种分工上；落地顺序是先按需加载建立索引，再压缩，之后分层，最后筛选。
...
- **筛选的反平衡**：筛选是“不进上下文”，不是“删掉来源”。[Guides 导航](../guides/index.md)同时写明“没有读取的来源不能声称已核对”，也不允许拿没读过的来源充当依据。
...
Skill 同样分层：`SKILL.md` 只放方法与边界，`references/` 按需读取（planning 的五份参考、delivery 的[双层循环协议](../../.agents/skills/evidence-delivery/references/lifecycle.md)），`tests/` 与 `evals/` 只在维护或明确评测时使用。双层循环本身也是分层：外层只处理任务选择、依赖与状态，内层只处理当前任务的执行质量，两层需要的上下文并不相同（见[下一讲](<上下文焦虑：Agent 在长对话中丢失关键信息.md>)）。
...
- **压缩的两条边界**：约束与验收标准不压缩——`AGENTS.md` 的不变量、CHECK 的 `acceptanceCriteria.expected`、gap 的 `statement`/`impact`/`nextAction` 原样保留；摘要可能出错，越重要的结论越要能回到原始命令输出与源文件。

### 演进触发条件

- 上下文预算是纪律与文档约定，不是门禁：`guides:verify` 只检查结构、本地链接与格式，`plan_state.py verify` 只检查计划结构、状态一致与 `done` 必须有证据，都不度量加载量，也没有单次读取上限。
- 大文件仍在增长（`api.json` 约 388 KB、`plan.yaml` 约 280 KB）；现在靠授权投影、紧凑清单与“只读相关章节”控制，还没有按 capability 或切片拆分的稳定分片索引。
- `openwiki/` 索引可能落后于源码；按需查阅后仍须回源码与测试核对。
