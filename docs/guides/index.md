...
| 问题                 | 权威位置                                                                                                         | 不可替代它的内容                 |
| -------------------- | ---------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| 业务事实             | [FM 源入口](../../.evidence/fm/model.json)、[发现记录](../../.evidence/discovery.md)（新访谈由发现流程按需创建） | 当前代码、生成报告、Agent 推测   |
...
2. **来源**：需求、FM/API、规则、场景、质量属性与架构引用是否可定位？当前源摘要与任务状态是否允许本次工作？
...
6. **验证与退出**：每条验收条件是否引用本任务 CHECK，并用稳定路径、受限操作符和保留类型的具体期望值表达？是否覆盖适用反例？失败后应局部修复、重规划还是澄清？
...
稳定基线由对应文档唯一维护；Skill 保存可移植的方法与固定技术 profile，项目文件记录本仓库具体取舍。编排入口只保留分支、输入输出、权限和退出条件；每个引用说明触发条件，专业方法由单一拥有者维护。任务只引用并说明局部差异，不复制整套指南。新增规范应指出对应测试/人工评估；`npm run guides:verify` 的实际覆盖见 [测试指南](../engineering/testing.md)。
