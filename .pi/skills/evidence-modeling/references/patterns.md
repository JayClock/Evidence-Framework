...
- **主管指派目标**：在这一事实分支中，目标设定的权利方为管理方，义务方为执行方。
- **执行方自报目标、主管审核**：目标设定的权利方与义务方反转；进度检查的方向仍单独确认，不随之全局反转。
...
领域部分本来就在 8X Flow 内，用同一 FM 的 Entity、Relationship、CEL Rule 表达。商品、内容、客户档案、场所等 Place/Thing 位于 Domain Context；稳定 Party 保持外部身份，通过 Role 扮演进入上下文。
