...
输入存在事实歧义时执行 `discovery-workshop.md`，按范围发现而非强制四阶段。候选不能冒充已确认事实。

四色原型映射：

- 粉色 moment-interval → Evidence；
- 黄色 role → Role；
- 绿色 party-place-thing → Participant；
- 蓝色 description → Evidence 条款、Thing 属性或 CEL Rule。
