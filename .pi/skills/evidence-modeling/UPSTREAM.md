...
3. `modeling/fm/files.ts` 白名单允许单层 discovery Markdown/YAML 与业务模式 YAML，拒绝派生文件；`modeling/fm/pipeline.ts` 统一执行业务模式文档生成，运行环境与发布分别由 `environment/` 和 `state/fm/` 提供。
...
8. 本地发现交互统一事实覆盖与提问决策：合同／凭证、领域身份、渠道、计算、异常及回放均先复用已有事实与确定性推导，不把技术映射或 FM 表达缺口转成业务问卷。新问题带稳定 gapKey，同一缺口不换 Q-ID；v5 拒绝缺 gapKey 的问题，不转换旧日志。新增只追加 resolution 关联原始事实、逐字摘录与问题，生成可重建 questionResolutions 视图，不生成 A-\*，不作人工决定。来源版本／文件、引用回答或原题后续回答变化使关联失效，修订／撤回按当前 D-ID，不自动恢复问答；摘录存在不等于语义证明。状态 v6、FM v3 保持不变；发现仅支持日志 v5，不兼容、迁移或改写旧日志／工件。
...
10. 本地模型更新采用 `references/incremental-assessment.md` 的 Context assessment v1 与事实级 discovery-coverage v1。问答只积累发现，人工 update-model 后按各类 Context 的本批次职责、具体事实依赖与阻塞影响评估 ready/support/pending；不要求关联 Context 整体完成。发布覆盖包含实际模型 ID、Context 类型及保留事实／剩余职责，成功后停回发现，人工另选 converge 才进入需求。删除候选 complete/dependencyRefs、缺 assessment、候选级覆盖、旧 finalizing 自动收敛和旧日志兼容分支；现有运行由人工决定后续处理，代码不迁移或重置数据。
