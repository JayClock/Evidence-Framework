...
- `evidence_ask_questions`：围绕候选上下文的一个相关缺口，每次 1–4 个面向业务的问题，带稳定 Q-ID、焦点、来源、影响和是否阻塞。问题写明依据与不确定点，不塞入整套问卷；调用后停止，状态为 waiting_answer。
- 用户通过 `/evidence-answer` 保存原文，扩展自动读取 `gh` 当前认证的 github.com 账号作为回答者，不再要求输入姓名；读取失败须修复 gh 登录或网络后重试，Agent 不代填身份。可部分回答、标注未知或有理由移出范围，也可按 Q-ID 更正。回答不是模型批准，账号归属不证明操作者实名、业务角色或批准权限。
