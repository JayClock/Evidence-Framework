...
问答只积累发现记录，不自动改正式统一语言或 FM。用户可以多轮回答，再手动选择「更新模型」（`/evidence-discovery update-model`）。`finish` 仅停止提问并整理；不是更新模型授权，也不自动定稿。`resume` 恢复问答。

人工选择更新后，先消化全部新增回答，再调用 `evidence_finalize_discovery`，必须提交覆盖全部历史有效候选的 Context assessment v1。Context 组织知识，不是不可拆分的完整包：Domain 评估身份、结构和相关规则；Channel 评估真实双方、凭证、有效性和本职责涉及的回应；Contract 评估身份、双方及相关约定；Fulfillment 评估请求、期限依据、确认凭证和判断规则。声明本批次 responsibility、具体 facts、requiredFactRefs、正常／边界／异常回放与 remainingScope，按 structure/provenance/decision 连接 consumerFactRef → providerFactRef；每道阻塞题定位到 affectedFactRefs。履约显式依赖父合同的相关双方和约定事实，不依赖全部条款、签约渠道或兄弟义务。

扩展计算 ready（本批次职责就绪）、support（只纳入被消费的已知支撑事实）、pending（本次不纳入）及事实级阻塞路径。候选整体 unknown 不否定其已知事实；相关 Context 未完成也可提供有来源的身份、金额或凭证。不能借拆分省略本职责真正需要的规则，也不要求全部领域生命周期或履约责任链闭合。没有可纳入职责也提交实际评估，不创造事实、不解除原题、不排除范围。协议与示例见 `references/incremental-assessment.md`。

本批次依次更新统一语言和 FM；先读取现有模型，保留稳定 ID，并说明新增、修订、撤回及待完善项。FM 提交 `discovery/formalization.md` 中唯一 discovery-coverage v1 JSON，记录当前 revision、全部 Context 的状态／模型映射／retainedFactRefs／remainingScope，以及每个纳入 factRef 的 modelRefs；发布前核对纳入与保留事实、实际模型 ID 和 Context 类型。支撑投影不扩大为整个 Context 完成，未纳入项仍保留。FM 校验失败不覆盖上一版 FM；成功后停回发现，等待人工继续问答或另选 `/evidence-discovery converge` 进入需求收敛。有新发现未更新、来源或已发布文件变化时不能直接收敛。
...
- 违约责任发现贯穿合同主线：主动逐项检查主要履约及新增补偿履约的违约后果，展示有依据的前序、触发条件、新履约请求与确认凭证，递增展开后续责任，有依据时记录法律边界或无后续义务的约定终点；这是持续发现方向，不是首版模型或独立主履约产出的全局准入条件。缺少约定只提出具体情景与责任缺口，不编造罚金或默认诉讼；自动终止且无额外义务不另造履约。按发现指南在 notes 保留各项覆盖、终点依据和未知，不把暂缓或范围外当作责任闭合；仍逐轮只问一个问题并服从人工停止。
...
- `evidence_finalize_discovery`：仅在人工选择更新模型后评估本批次，按上节的全历史 assessment 判定纳入部分与缺口；成功仅启用统一语言／FM，不自动进入软件需求。包括简单胶水在内，assessment 始终必填；不适用须 contexts 为空、有来源理由且没有未解决阻塞题。没有业务依据时保留缺口，不能用“不适用”逃避信息不足。
...
```text
人工更新模型：统一语言 → 统一 FM → 停回发现（可重复）
人工进入需求收敛：用户/角色需求 → 问题与 MVP → 故事及验收目录 → Gate
```
...
- FM 更新成功后停回发现，不立即进入需求收敛或产生 Gate；由人工选择 converge，基于最近成功更新且与发现一致的模型进入需求收敛。业务模型中的线下、外部和人工活动不自动变成开发需求。不存在旧 finalizing 自动进入需求的兼容路径。
- 需求收敛按 `evidence-requirements` 的方法说明与当前模板，从模型、回放案例及真实痛点提取本次软件职责、MVP、稳定 US/AC 和预期结果。保留追溯，不按数量补造人物、Epic 或故事。
- 定稿中发现矛盾，先用 `evidence_save_discovery` 记录并重开发现；旧定稿就绪结果失效，上一版已发布 FM 保留。人工停止状态不得直接提问；由人工 resume 后再澄清，并手动触发下一次更新。不私改其他工件。进入下游后须先人工回退 Modeling。
...
发现日志仅支持 v5，拒绝旧日志／旧评估／旧覆盖协议，不迁移、不自动转换或重置已有运行；旧运行由人工决定归档或重新初始化，不能手改版本号。每次交互追加一个 `artifacts/02-modeling/discovery/<runId>/revision-N.json`，只保存本轮事件及前序摘要，不重复完整现状。问题、人工回答（含前次 A-ID）、Agent 发现记录、人工控制和机器检查分别留痕。当前完整视图由扩展校验整条摘要链并重放生成，不作为另一份事实源。D-ID 格式为 D-版本号-记录序号（均至少三位），由扩展分配；当前视图由扩展自动提供，按需用 read 读取，不直接写盘。正式需求文档仍保存到 `artifacts/01-requirements/`，目录名不是独立阶段。
