...
import { latestAnswer } from './discovery-questions.ts';
...
async function validateResolution(
  ...
  if (
    value.sourceRefs.some(
      (ref) => !value.citations.some((citation) => citation.sourceRef === ref),
    )
  )
...
export async function validateResolutionRecords(
  root: string,
  snapshot: DiscoverySnapshot,
...
export async function markChangedResolutionSources(
  root: string,
  snapshot: DiscoverySnapshot,
): Promise<string[]> {
...
export async function assertResolutionSelectionFresh(
  root: string,
  snapshot: DiscoverySnapshot,
  questionId?: string,
): Promise<void> {
  const changed = await markChangedResolutionSources(root, snapshot);
  if (changed.some((key) => !questionId || key === `resolution:${questionId}`))
    throw new Error(
