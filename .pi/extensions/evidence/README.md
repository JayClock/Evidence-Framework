...
工具、日志、人工操作与提交路径差异只在 [执行适配](instructions/modeling-adapter.md) 和 [评估协议](instructions/incremental-assessment.md) 维护，不写回独立包。FM 回归与生成评测在 `.agents/skills/evidence-fm/` 内维护，各项业务行为评测随对应 Skill；跨 Skill 检查在 `.agents/skills/fm-modeling/tests/`，Pi 交互评测在 `.agents/skills/evidence-discovery/evals/pi-discovery/`，升级不改运行状态、既有工件或 Gate。Architecture、Planning、TDD 与 Review 工序也统一由 `.agents/skills/` 提供。
