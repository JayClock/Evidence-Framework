...
- 用统一 FM Schema v3 表达领域、渠道与履约，按需模拟单据、派生业务模式、编译并原子保存；DDD 工件为设计投影。
...
- `/evidence-init` 保存原始需求后直接开始用户画像工件；Domain 顺序为统一语言 → 统一 FM → DDD 限界上下文映射 → 实体/值对象 → 聚合 → 领域事件。
- 无独立访谈、回答命令或基线确认步骤。业务疑点在工件中列明，使用 `/evidence-review` 或 `/evidence-revise` 处理，不把推断当成已确认事实。
- 工作流状态版本为 4，不兼容版本 1/2/3：Domain 工件顺序与 FM 适用性已改变，不能沿用旧索引、跳过决策或复用旧 Gate。更新后运行 `/reload`；旧运行先备份，再 `/evidence-reset`、`/evidence-init`，不要手改版本号。本地无活动运行时直接初始化即可。
- `.pi/evidence.json` 配置版本仍为 1；FM 升级到 Schema v3，拒绝 v2。纯领域/纯渠道允许无履约，只有无独立语义的简单胶水才可不建模。模型人工状态、机器校验、实际单据模拟和 Domain Gate 保持独立。
...
- 升级后执行 `/reload`；版本 1/2/3 按 [升级说明](../../../docs/evidence.md#22-破坏性升级) 重置并重新初始化。不要改版本号或复用旧 Gate；六个宏观阶段、配置版本及 Q1/Q2 测试契约不变。
