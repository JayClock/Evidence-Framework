            ...
            Type.Object(
              {
                ...
                deadline: knownText(
                  '保留已识别请求的类型语义，如“以本次付款请求的截止时间（expired_at）为准；确定依据待明确”。按四色追溯业务来源：已知直接约定、引用或派生规则就保留；来源未知须标明，影响判断时澄清，不默认输入或公式。字段非空不表示期限依据已解决；仅全未知为 null，不因缺公式清空已知结构。不编造时长、实例日期、无期限或设定权限。',
                ),
