...
该项目级 Pi 扩展只提供一个命令和一个问答工具。Agent 按 Skill 直接编辑项目文件、运行校验并在普通对话中展示差异；扩展不维护业务状态、不读写模型、不执行版本操作。外层交付循环由 `smart-domain-task-planning`、`smart-domain-delivery` 和仓库中的计划文件承担，不在扩展中增加自动推进或私有状态。
...
建模完成后也不会自动进入实施。用户另行要求规划时，由 `smart-domain-task-planning` 生成 `docs/plans/smart-domain/`；用户要求实施或恢复时，由 `smart-domain-delivery` 读取任务 DAG、执行一个就绪任务并在完成后停止。
