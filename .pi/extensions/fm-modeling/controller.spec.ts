...
describe('ModelingController first turn', () => {
  it('records input before starting exactly one modeling turn', async () => {
    ...
    await new ModelingController(pi as never, store).handle(
      '  作者结算需求  ',
      ctx as never,
    );
    ...
    expect(state).toMatchObject({
      runId: expect.stringMatching(/^FM-\d{4}-001$/),
      revision: 1,
    });
    ...
    expect(setActiveTools).toHaveBeenCalledWith(
      expect.arrayContaining(['read', 'fm_model_submit']),
    );
  ...
  it('does not overwrite an active run or start from empty input', async () => {
    ...
    expect((await store.readEvent('FM-2026-001', 1)).event).toMatchObject({
      text: '原需求',
    });
  });
  ...
  it('stops immediately while waiting and reports that no model exists', async () => {
    ...
    expect(
      (await store.readEvent(state!.runId, state!.revision)).event.kind,
    ).toBe('interaction-stopped');
    expect(notify).toHaveBeenCalledWith(
      expect.stringContaining('当前没有有效 FM 模型'),
      'warning',
    );
  });
