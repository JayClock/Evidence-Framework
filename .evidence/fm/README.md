...
- [订阅合同](contexts/subscription/contract.json)、[移动支付协议](contexts/mobile/contract.json)、[预付费账户协议](contexts/prepaid/contract.json)及[内容领域](contexts/content/context.json)分别保留原有业务边界。
...
- 各履约目录以 `context.json` 表达责任边界，`request.json`、实际存在的 `confirmation.json`、`evidence/`、`roles/` 和 `rules/` 表达该履约的凭证与规则。订阅付款通过证明角色判断结果，不补造本地确认类型。
