...
扩展不拥有这些状态。Skill 读取并按授权维护文件，确定性脚本只观察和报告。项目导航只定位来源，不是第三份状态；任务前馈未就绪时不得从结构候选直接进入 Action。
...
- 当前任务的授权、业务/工程来源、设计、做法/环境和 CHECK 是否足够开工。
- 哪些业务源、架构、规范、howto、代码或环境变化使旧证据失效；不以 FM/API 摘要代替全部新鲜度判断。
...
1. 读取项目宪法、Guides 导航和 Git 差异，确认合法模式与授权范围；
2. 核对当前 FM/API 与计划输入，必要时回到 Plan 重编译；
3. 运行 `plan_state.py verify`，再读取 `next` 的结构候选；
4. 选择一个任务并读取其 Guides、局部设计与 CHECK；
5. 加载直接前置证据、sourceRefs 与 procedureRefs 对应的实际业务/工程来源；
6. 检查授权/目标、来源/审核、前置新鲜度、设计边界、做法/环境及验证/退出是否就绪；
7. 确认当前工作树与任务状态一致，通过后才开始本任务。架构/指南/代码/环境变化须评估重验，不能只依赖业务源摘要。
