...
| 当前任务                           | 读取                                                                |
| ---------------------------------- | ------------------------------------------------------------------- |
| 访谈中澄清术语、增量维护统一词汇表 | [领域语言沉淀](domain-language.md)                                  |
...
访谈按领域语言方法当轮更新明确术语，并按需读取其他专业参考；术语写入不启动模型 JSON 生成。问题选择、回答等待和控制状态由访谈任务管理，本包输入复核只输出具体缺口。

每项规则在本包按职责维护：结构判断看 business-analysis，业务来源看 provenance，时间与 JSON 结构看 format，运行实例与 CEL 依赖看 traceability-and-simulation，执行结果记录看 validation。类型与机器结果不替代业务来源或业务确认依据。Schema v3 的实际格式限制以 format 为准。
