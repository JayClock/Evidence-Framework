...
- `valid`：类型、语义、CEL、lineage 和提交的 validation 套件没有检测到错误。
...
需要正式更新、交接验证结论或提交审核时，在项目已有检查记录中保存一次运行依据；临时排错可以只查看输出，不默认增加每轮日志文件。没有写入授权时在回复中给出结果并说明未保存。
...
下列命令只在允许的候选目录中执行；源 YAML 才是正式类型事实源。
...
- 校验失败保留旧版；发布前确认来源及目标未被并发修改；保留可恢复副本，不承诺宿主没有的事务能力。
