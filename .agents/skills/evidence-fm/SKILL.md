...
- 读取当前来源、术语、相关发现／问题／案例及现有 FM。新产物默认统一放在项目根的 `.evidence/`：正式模型、术语、场景及模型派生输出使用 `.evidence/fm/`，编辑候选使用 `.evidence/fm-candidates/<批次>/`，发布 CLI 的冻结准备结果使用 `.evidence/.fm-work/`，检查与发布记录使用 `.evidence/fm-checks/`。候选、目标和发布工作目录不得相同或互相包含。沿用已有文件与用户指定位置，不自动迁移；命令用绝对路径，不向安装目录写项目输出。
