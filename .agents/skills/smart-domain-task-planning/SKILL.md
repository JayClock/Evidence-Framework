---
name: smart-domain-task-planning
...
定位项目指令、当前变更和合法工作阶段。读取全部 FM 源 Context、实体、关系、规则、场景及引用实例，保留审核状态与未决事实；生成目录仅作参考。
...
- 详情包含来源、设计/非目标、依赖产物用法、真实文件范围、步骤、精确检查命令、完成条件与实际证据。身份仅作绑定，不复制依赖图、状态或 API 覆盖数组。
...
- 新会话只需索引、本任务材料与相关依赖，不依赖聊天记忆。保留已有 US/AC/TP/Rule/Scenario/API 标识，不制造不存在的上游编号。
...
输出计划后停止，不自动实现或审批。活动流程的正式 Planning 工件仍使用宿主模板与提交机制。
