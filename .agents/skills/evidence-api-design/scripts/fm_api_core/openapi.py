...
class _IndentedSafeDumper(yaml.SafeDumper):
    ...
    def choose_scalar_style(self) -> str:
        """Match repository formatter quoting so generated bytes stay stable.

        Quoted scalars use single quotes unless the text contains a single quote
        (and no double quote), where double quotes avoid escaping; multi-line or
        otherwise unquotable scalars keep the base emitter choice.
        """
        style = super().choose_scalar_style()
        analysis = getattr(self, "analysis", None)
        value = getattr(getattr(self, "event", None), "value", None)
        if (
            analysis is None
            or not isinstance(value, str)
            or style not in {"'", '"'}
            or not analysis.allow_single_quoted
        ):
            return style
