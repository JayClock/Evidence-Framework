...
def build_openapi(projection: dict[str, Any]) -> dict[str, Any]:
    ...
    document: dict[str, Any] = {
        ...
        "paths": {},
        "components": {"schemas": {}},
        "x-generated-from": "evidence-api-design projection.json",
        "x-evidence-api-design-schema-version": projection["schemaVersion"],
...
def render_openapi(projection: dict[str, Any]) -> str:
    return (
        json.dumps(
            build_openapi(projection), ensure_ascii=False, sort_keys=True, indent=2
        )
        + "\n"
    )
