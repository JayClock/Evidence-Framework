...
project 的 out 必须位于项目根内、父目录已存在，并且不能与 FM、设计或来源重叠；默认值为 `.evidence/api/generated/`。先在内存形成全部内容，再在输出同级临时目录写出并同步全部文件，最后目录级替换；已有目录在切换失败时恢复，成功后删除旧目录及其中过期产物。版本历史由 Git 管理，不通过批次目录保留。
