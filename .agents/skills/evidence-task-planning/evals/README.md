...
[evals.json](evals.json) 覆盖纯领域、跨上下文证明、API共享身份、不完整映射、通用后端模块与普通实现、直接复用分页、业务模块封装与本地事务，以及项目前馈装配和无固定导航时的可移植消费。它们是待执行的行为评估定义，不是已通过报告。

使用新的隔离会话/可用子Agent，提供相同合成输入和固定Skill版本；输出保存到独立评估目录。没有独立运行条件时明确未执行，不制造基线、评分、token、时间或通过结论。

人工重点核对：

- 是否按项目宪法、项目基线、工程指南和任务 Guides 选择最小相关来源，不复制知识库或范例业务，不新增前馈状态。
- 是否核对工程来源/环境变化对证据的影响，而不只比较 FM/API 摘要；不存在必要来源时局部阻塞，缺少约定导航文件名不自动阻塞等价资料充分的工作。
