...
- 工序选择与切片测试策略：{{实际工序指南/测试策略的路径与章节、触发与未触发的工序、每项结果的 Q2 → Q1 支撑、按架构组件划分的功能上下文与替身选择、合并与拆分理由；具体实例与 CHECK 在任务详情，不在索引复制步骤}}
- 执行模式：{{根据本次授权在 taskNotes 选择 implementation/verify/design/setup/manual；工序不等于 mode}}
...
- 工序按具体变化触发，不按实体/场景复制全套任务；单元唯一拥有、适配依赖契约、真实装配汇合，不凭技术层排序添加依赖。工序未触发不意味着可以排除已有 API 能力。
