...
《Agent 驾驭工程之美》强调：状态是前馈与反馈之间、内层与外层之间的桥梁。本项目采用“仓库即规范”，不在扩展中维护隐藏工作流状态。
...
环境不可用时任务进入 `blocked`，不能把安装失败、命令错误或配置缺失记录成行为 Red。
