...
- **专栏订阅平台**：读者与专栏运营方订立订阅合同，贯通订阅费支付、付费内容访问、断更退款与重新上架后的免费恢复；移动支付与预付费抵扣分别以具体凭证证明资金结果。业务澄清见[发现记录](.evidence/discovery.md) B01–B09。
- **CRM 电话销售绩效协议**：绩效管理者与电话销售就一个业务周期内的客户联系目标形成内部权责约定。管理者提出带周期及联系总数、电话数、邮件数的月度目标请求；电话销售每次实际联系形成客户联系记录，记录指向独立客户信息领域中的客户档案并注明渠道；三项目标分别达到才完成履约，未达标只表示完成条件不成立。

两个切片在同一 FM 与同一 API 设计源中各自独立表达，不互相引用；业务事实以 [FM 概览](.evidence/fm/00-overview.md)为准。

| 切片             | 当前状态                                                                                                                                                        |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 专栏订阅平台     | FM/API/计划已恢复为当前来源（提交 `0e76410` 曾移除，2026-09-20 按用户指令从 Git 历史恢复）；计划 14 个任务中属于该切片的 8 个：2 个基础任务已完成、6 个 planned |
| CRM 绩效协议     | FM/API/计划与 local/test 实现已完成；三个登记能力与月度完成规则按完成、不足两条旅程验收                                                                         |
| 本地用户基础切片 | 独立于两个业务的 ID / displayName CRUD 与分层测试                                                                                                               |
...
README 的主线不是按目录介绍代码，而是展示一次业务能力如何在 PDCA 中逐步变成可验证的软件交付。下面用已交付的绩效协议切片走一遍四个阶段；专栏订阅平台切片同样沿这条主轴推进，其 FM/API/计划已恢复为当前来源，6 个实施任务等待继续授权。每个阶段都有输入、产物、检查点和截图；阶段结束后才把结果交给下一阶段。
...
[FM 源](.evidence/fm/model.json) 不保存审核状态；业务确认依据由发现记录与需求/计划记录承载。当前 FM 与 API 同时描述专栏订阅平台与电话销售绩效协议两个独立切片，不等于已上线业务系统。
